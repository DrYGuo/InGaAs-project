function [ratio,ratio2,log_ratio,U200]=Blochwavetrial2(FPARAMS,ABSratio,structure)
%--------------------TILTE----------------------------------
...
%a more detailed derivation of the Blochwave approach is given.
...
E0=203*1e3; % accelrating voltage
...
for i=3:m
    if structure(i,1)~=structure(i-1,1)
     nthatom=nthatom+1;
    Ztable(nthatom,:)=[nthatom structure(i,1)];
    end 
end

% while(structure(i,1)~=structure(i-1,1))
%     nthatom=nthatom+1;
%     Ztable(nthatom,:)=[nthatom structure(i,1)];
%     
% end
...
Ug_struct.numberofrow=numberofrow;

U0=Ug([0 0 0],Ug_struct);
[U200,V200,F200]=Ug([2 0 0],Ug_struct)

K0=sqrt((1/lambda)^2+U0^2); %the magnitude of the wave vector inside the crystal
ZA=[0 0 1]; %zone axis;
SN=[0 0 1];%surface normal
Kt=[0 22 0]; %the projection of the wave vector Kt=Kt1*astar+Kt2*bstar+Kt3*cstar Kt=[Kt1,Kt2,Kt3]
...
SNK= (MetricR*SN')';%-(MetricR*SN')';% the negative sign means that the surface normal is against the incident beam
...
% setting Sg([1 11 1])=0 returns Kt
y0=Kt0(2);
dy=2;
Kt1=Kt0;
for i=1:40
...
Sglower=Sg([1 11 1],Sg_struct);% Sg
Sglower=real(Sglower);
...
Sgupper=Sg([1 11 1],Sg_struct);
Sgupper=real(Sgupper);
if Sgupper*Sglower<0
   dy=dy/2;  
end
...
Sg_boundary=[Sglower,Sgupper];
% disp(Sg_boundary);
end
Kt=Kt1;
Kt0=Kt1;
 disp(Kt);
 disp(real(Sg([1 11 1],Sg_struct)));
...
% Kt and g (both are in a* b* c*)
K=Kt+Kz;
%excitation_error=10*Sg([1 11 1])% in the unit of nm-1
Sg_struct.K0=K0;
Sg_struct.Metric=Metric;
Sg_struct.K=K;
...
kmax=40;
lmax=40;
Sgmax=0.05; % the maximum excitation error
for hh=-hmax:hmax
    for kk=-kmax:kmax
        for ll=-lmax:lmax
            ...
            if (ZA*g'>=0)&&(ZA*g'<=NHOLZ) && (abs(Sg(g,Sg_struct))<=Sgmax)&&(mod(hh+kk,2)==0)&&(mod(hh+ll,2)==0 && abs(hh)+abs(kk)+abs(ll)>0)%&&(mod(hh+kk,2)==0)&&(mod(hh+ll,2)==0 is to include the hkl which are all even or all odd so this condition is not neccessary in general; && abs(hh)+abs(kk)+abs(ll)>0 is to exclude the central beam(0 0 0)
               ...
               checklist(count,7)=1+dot(SNK.*[astar bstar cstar],g.*[astar bstar cstar])/dot(SNK.*[astar bstar cstar],K.*[astar bstar cstar]);
               %dot(SNK.*[astar bstar cstar],(K+g).*[astar bstar cstar])/(sqrt((K+g)*Metric*(K+g)')*sqrt(SNK*Metric*SNK'));
            end
        end
    end
end
...
Bijvoet1=[-1 11 1]; % input the Bijvoet pair
Bijvoet2=[1 11 1];  % input the Bijvoet pair

index_table=Intensity(:,1:3);

for row=1:N
    hkl=index_table(row,:);
    if (hkl(1)==Bijvoet1(1))&&(hkl(2)==Bijvoet1(2))&&(hkl(3)==Bijvoet1(3))
       rowB1=row;
    end
   if (hkl(1)==Bijvoet2(1))&&(hkl(2)==Bijvoet2(2))&&(hkl(3)==Bijvoet2(3))
       rowB2=row;
    end
end
% disp(rowB1);
...
for row=1:2*Np_disc+1
    for col=1:2*Np_disc+1
        I1_total(n)=I1_total(n)+Ig(row,col,n,rowB1);
        I2_total(n)=I2_total(n)+Ig(row,col,n,rowB2);
        ratio(n)=I1_total(n)/I2_total(n);
        ratio2(n)=(I1_total(n)-I2_total(n))/(I1_total(n)+I2_total(n));
        log_ratio(n)=log(ratio(n));
    end
end
